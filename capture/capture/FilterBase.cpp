...
// Implement IEnumPins.
class PinEnumerator
	: public IEnumPins, public RefCount<PinEnumerator> {
public:
	...
	STDMETHOD(Next)(ULONG count, IPin** pins, ULONG* fetched) {
		...
		while (pins_fetched < count && filter_->NoOfPins() > index_) {
			...
			LOG(INFO) << " to get next count = " << count << " index_ " << index_  << " pin " <<pin <<" filter_ " << filter_.get();
			if (pin == NULL) {
				continue;
			}
			pin->AddRef();
	...
	STDMETHOD_(ULONG, AddRef)() {
		RefCount<PinEnumerator>::AddRef();
		return 1;
	}

	STDMETHOD_(ULONG, Release)() {
		RefCount<PinEnumerator>::Release();
		return 1;
	}
...
ULONG STDMETHODCALLTYPE FilterBase::AddRef() {
	RefCount<FilterBase>::AddRef();
	return 1;
}

ULONG STDMETHODCALLTYPE FilterBase::Release() {
	RefCount<FilterBase>::Release();
	return 1;
}
