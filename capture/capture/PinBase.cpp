...
class TypeEnumerator
	: public IEnumMediaTypes, public RefCount<TypeEnumerator> {
public:
	...
	STDMETHOD_(ULONG, AddRef)() {
		LOG(INFO) << " TypeEnumerator pin  to add ";
		RefCount<TypeEnumerator>::AddRef();
		return 1;
	}

	STDMETHOD_(ULONG, Release)() {
		//xplatform_util::RefCounted<TypeEnumerator>::Release();
		LOG(INFO) << " TypeEnumerator pin  to release ";
		RefCount<TypeEnumerator>::Release();
		return 1;
	}
...
PinBase::PinBase(IBaseFilter* owner)
	...
	LOG(INFO) << " @@@@filter = " << owner << " pin = " << this;
 }

PinBase::~PinBase() {
	LOG(INFO) << " ~~~ PinBase " << this;
}
...
STDMETHODIMP PinBase::Connect(IPin* receive_pin,
	...
	LOG(INFO) << "==============Pin base:" << this <<"  receive_pin " << receive_pin;
...
STDMETHODIMP PinBase::ReceiveConnection(IPin* connector,
	const AM_MEDIA_TYPE* media_type) {
	LOG(INFO) << "========Pin base:" << this <<" connector " << connector;

	if (!IsMediaTypeValid(media_type)) {
		LOG(INFO) << "return VFW_E_TYPE_NOT_ACCEPTED because media type doesn't acceptable.";
...
STDMETHODIMP PinBase::Disconnect() {
	if (!connected_pin_)
		return S_FALSE;
	LOG(INFO) << " Disconnect  " << connected_pin_;
...
STDMETHODIMP_(ULONG) PinBase::AddRef() {
	...
	RefCount<PinBase>::AddRef();
	return 1;
}

STDMETHODIMP_(ULONG) PinBase::Release() {
	RefCount<PinBase>::Release();
	return 1;
}